...
export class CreateSpaceModal {
    ...
    constructor(page: Page) {
        ...
        this.newSpaceNameInput = page.getByPlaceholder(
            "e.g. Marketing, Engineering, HR"
        );
        this.continueButton = page.locator(
            "[data-test=create-space-details__continue-button]"
        );
        this.createSpaceButton = page.locator(
            "[data-test=create-test-workflow__create-space-button]"
        );
    }
    ...
    async typeIntoNewSpaceNameInput(text: string) {
        return await test.step(`Typing '${text}' into new space name input`, async () => {
            ...
            CustomReporter.logAction(
                `Typed '${text}' into new space name input`
            );
    ...
    async clickOnCreateSpaceButton() {
        return await test.step(`Clicking on 'Create Space' button on modal`, async () => {
            ...
            CustomReporter.logAction(
                `Clicked on 'Create Space' button on modal`
            );
            await allure.attachment(
                "screenshot.png",
                await this.page.screenshot(),
