...
export class Hooks {
    ...
    static async createSpaceByName(request: APIRequestContext, spaceName: string): Promise<string> {
        const teamId = process.env.BASE_TEAM_ID as string;
        const endpoint = spaceEndpoint.replace("TEAM_ID", teamId);
        const apiKey = AuthService.getApiKey();
    ...
    static async createListByName(request: APIRequestContext, spaceId: string, listName: string): Promise<string> {
        const teamId = process.env.BASE_TEAM_ID as string;
